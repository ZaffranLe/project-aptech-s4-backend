...
{
	public class PermissionSql : EntityBaseSql 
	{
		...
		private void Init()
		{
           Insertsql = "Permission_Insert";
           Updatesql = "Permission_Update";
           Deletesql = "Permission_DeleteByPrimaryKey";
           Selectsql = "Permission_SelectAll";
		}
        ...
        public override SqlCommand GetSqlCommandForInsert(BaseEntity baseEntity)
		{
            ...
            if (baseEntity != null)
            {
                var businessObject = baseEntity as Permission;
		...
		public override List<BaseEntity> PopulateBusinessObjectFromReader(IDataReader dataReader)
        {
            ...
            {
                var businessObject = new Permission();
    ...
    internal void PopulateBusinessObjectFromReader(Permission businessObject, IDataReader dataReader)
        {
            if (_dicIndex.Count == 0) FillDicIndex(dataReader);
		
			if (GetIndex(Permission.PermissionFields.CreatedAt.ToString()) != -1)
				if (!dataReader.IsDBNull(GetIndex(Permission.PermissionFields.CreatedAt.ToString())))
				{
					businessObject.CreatedAt = dataReader.GetDateTime(GetIndex(Permission.PermissionFields.CreatedAt.ToString()));
				}

			if (GetIndex(Permission.PermissionFields.CreatedBy.ToString()) != -1)
				businessObject.CreatedBy = dataReader.GetInt32(GetIndex(Permission.PermissionFields.CreatedBy.ToString()));

			if (GetIndex(Permission.PermissionFields.Description.ToString()) != -1)
				businessObject.Description = dataReader.GetString(GetIndex(Permission.PermissionFields.Description.ToString()));

				businessObject.Id = dataReader.GetInt32(GetIndex(Permission.PermissionFields.Id.ToString()));

			if (GetIndex(Permission.PermissionFields.Name.ToString()) != -1)
				businessObject.Name = dataReader.GetString(GetIndex(Permission.PermissionFields.Name.ToString()));

			if (GetIndex(Permission.PermissionFields.UpdatedAt.ToString()) != -1)
				if (!dataReader.IsDBNull(GetIndex(Permission.PermissionFields.UpdatedAt.ToString())))
				{
					businessObject.UpdatedAt = dataReader.GetDateTime(GetIndex(Permission.PermissionFields.UpdatedAt.ToString()));
				}

			if (GetIndex(Permission.PermissionFields.UpdatedBy.ToString()) != -1)
				businessObject.UpdatedBy = dataReader.GetInt32(GetIndex(Permission.PermissionFields.UpdatedBy.ToString()));
