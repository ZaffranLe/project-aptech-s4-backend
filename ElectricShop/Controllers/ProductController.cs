...
namespace ElectricShop.Controllers
{
	public class ProductController: ApiController
	{
		...
		public async Task<IHttpActionResult> Get()
		{
			try
			{
                ...
                foreach (var product in lstData)
			    {
			        ...
			        {
                        Product = product,
                        ListImagesUrl = ImagesUtils.GetImagesUrl(product.ImageId)
			        });
                }
		...
		[EnableCors(origins: "*", headers: "*", methods: "*")]
		public async Task<IHttpActionResult> Post([FromBody]Product req)
		{
			try
			{
		...
		#region Validation
		private bool Validate(Product obj, out string errorCode, out string errorMess)
		{
			errorCode = null;
			errorMess = null;
			try
